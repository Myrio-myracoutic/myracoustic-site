  ...
  return (
    <div style={{ padding: '36px 36px 60px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 28 }}>
        <div>
          <h1 style={{ fontFamily: 'var(--font-display), sans-serif', fontSize: 26, fontWeight: 800, color: '#111827', margin: '0 0 4px' }}>Devis</h1>
          <p style={{ color: '#9ca3af', fontSize: 14, margin: 0 }}>{events.length} devis au total</p>
        </div>
        {/* Recherche */}
        <input
          value={search}
          ...
          style={{
            ...
            padding: '9px 16px', fontSize: 14, color: '#374151',
            outline: 'none', width: 260, boxShadow: '0 1px 2px rgba(0,0,0,0.05)',
          }}
        />
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>
        {FILTERS.map(f => (
          <button key={f.key} onClick={() => setFilter(f.key)} style={{
            background: filter === f.key ? '#060e16' : '#fff',
            border: `1px solid ${filter === f.key ? '#060e16' : '#e5e7eb'}`,
            color: filter === f.key ? '#b8ef0b' : '#6b7280',
            ...
            return (
              ...
              >
                <div>
                  ...
                  <p style={{ fontSize: 12, color: '#9ca3af', margin: 0 }}>{ev.clients?.email}</p>
                </div>
                <div>
                  <p style={{ fontSize: 14, color: '#374151', margin: '0 0 2px' }}>{ev.event_type || '—'}</p>
                  <p style={{ fontSize: 12, color: '#9ca3af', margin: 0 }}>{ev.venue_city || ''}</p>
                </div>
                <p style={{ fontSize: 13, color: '#374151', margin: 0 }}>{fmtDate(ev.event_date)}</p>
                <p style={{ fontSize: 13, color: '#374151', margin: 0 }}>{ev.guests ? `${ev.guests} pers.` : '—'}</p>
                <span style={{
                ...
                <span style={{ color: '#d1d5db', fontSize: 16, textAlign: 'center' }}>›</span>
              </div>
            );
          })
        )}
