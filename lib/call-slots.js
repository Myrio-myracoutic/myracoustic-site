...
export async function getAvailableSlots(dateStr, windowDays = BOOKING_WINDOW_DAYS) {
  ...
  // Garde-fou supplémentaire : autres fiches déjà réservées ce jour-là (les 3 tables qui
  // portent un planning d'appel — mariage, devis particulier/pro, contact pro simple).
  const dayStart = parisLocalToUtcISO(dateStr, '00:00');
  const dayEnd = parisLocalToUtcISO(dateStr, '23:59');
  const CALL_TABLES = ['mariage_leads', 'qonto_quotes_tracking', 'pro_contact_leads'];
  const bookedRowsByTable = await Promise.all(CALL_TABLES.map(table => supabaseAdmin
    .from(table)
  ...
  const bookedTimes = new Set(bookedRowsByTable.flatMap(({ data }) => (data || []).map(r => new Date(r.call_scheduled_at).getTime())));
