...
function BulletItem({ children }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 14 }}>
      <WaveBullet size={16} />
      <span style={{ color: 'rgba(255,255,255,0.72)', fontSize: 15, lineHeight: 1.65 }}>{children}</span>
    </div>
  );
}
    ...
    >
      <div style={{ padding: '32px 28px', flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <span style={{
            fontFamily: 'var(--font-display), sans-serif', fontSize: 11, letterSpacing: '0.2em',
        ...
        <h3 style={{ fontFamily: 'var(--font-display), sans-serif', fontSize: 24, fontWeight: 700, marginBottom: 10 }}>{title}</h3>
...
function FaqItem({ q, a }) {
  ...
  return (
    <div style={{
      ...
      {open && (
        <p style={{ margin: 0, padding: '0 22px 20px', color: 'rgba(255,255,255,0.55)', fontSize: 14, lineHeight: 1.7 }}>{a}</p>
      )}
    </div>
  );
}
...
const TESTIMONIALS = [
  ...
  { name: 'Sandra et Stéphanie', event: 'Mariage', stars: 5,
    text: "Myrio a été à l'écoute de nos envies, on a tout calé ensemble. Ces propositions ont été pertinentes. Nous l'avions déjà vu comme dj dans des soirées repas d'entreprise et comme nous l'avions apprécié nous l'avons contacté pour notre mariage." },
];
...
export default function MariageCityClient({ city }) {
  ...
  return (
    <div style={{ paddingTop: 70 }}>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', position: 'relative', zIndex: 2 }}>
          ...
          <h1 style={{
            ...
            DJ MARIAGE<br /><span style={{ color: 'var(--lime)' }}>À {nom.toUpperCase()}</span>
          </h1>
          <p style={{
      ...
      {/* ── STATS ───────────────────────────────────────────────── */}
      <section style={{ padding: '72px 32px', background: '#060e16', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <Reveal style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 32 }}>
          <StatItem value="26" suffix="+" label="Années d'expérience" />
          <StatItem value="200" suffix="+" label="Mariages animés" />
          <StatItem value={distance} suffix="" label={`de ${nom}`} />
        </Reveal>
      </section>
      ...
      <section style={{
        ...
        <Reveal style={{
          ...
          <div>
            ...
            <h2 style={{ fontFamily: 'var(--font-display), sans-serif', fontSize: 'clamp(24px,3vw,40px)', fontWeight: 700, marginBottom: 32 }}>
              Ce que vous <span style={{ color: 'var(--lime)' }}>obtenez</span>
            </h2>
            {INCLUDES.map((item, i) => <BulletItem key={i}>{item}</BulletItem>)}
          </div>
          <div style={{ borderRadius: 16, minHeight: 380, backgroundImage: 'url(/ban_fête_myr.jpg)', backgroundSize: 'cover', backgroundPosition: 'center' }} />
        </Reveal>
      </section>
      ...
      <section style={{ padding: 'clamp(56px,7vw,88px) 32px' }}>
        <Reveal style={{ maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-display), sans-serif', fontSize: 'clamp(24px,3.5vw,42px)', fontWeight: 700 }}>
      ...
      <section style={{ padding: 'clamp(56px,7vw,88px) 32px' }}>
        <Reveal style={{ maxWidth: 760, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-display), sans-serif', fontSize: 'clamp(24px,3.5vw,42px)', fontWeight: 700, marginBottom: 12 }}>
              DJ Mariage à {nom} : vos questions
            </h2>
          </div>
      ...
      <section style={{
        ...
        <h2 style={{ fontFamily: 'var(--font-display), sans-serif', fontSize: 'clamp(24px,3.5vw,44px)', fontWeight: 700, marginBottom: 12 }}>
          Disponible pour votre date à {nom} ?
        </h2>
