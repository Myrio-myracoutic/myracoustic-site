...
export default function MariageContactClient() {
  ...
  useEffect(() => {
    const fireAbandon = () => {
      if (!startedRef.current || submittedRef.current || abandonFiredRef.current) return;
      ...
      {step === 'slot' ? (
        ...
        <div style={{ maxWidth: 620, margin: '0 auto', padding: '40px 20px 70px' }}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <MiniCal selected={date} onSelect={(k) => { touch(); setDate(k); }} onBookedClick={(k) => { gtagEvent('date_indisponible', { profil: 'mariage', step_name: 'formulaire_contact' }); setBookedNotice(k); }} onPendingClick={(k) => { touch(); setDate(k); }} devisPending={pendingDates} bookedDates={bookedDates} loading={availLoading} yearsAhead={2} />
