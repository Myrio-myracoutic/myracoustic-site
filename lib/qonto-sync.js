...
export async function runSync() {
  ...
  for (const inv of invoices) {
    ...
    // L'enregistrement du paiement est TOUJOURS tenté, indépendamment du blocage d'agenda — bug
    // réel trouvé le 19/08/2026 : une facture supplémentaire sans date d'événement (ex. "heures
    // supplémentaires" facturées à part, 100€ chez Carine Grelier) se faisait sauter entièrement
    // par l'ancien test "calendrier déjà bloqué pour cette facture" placé avant l'enregistrement
    // du paiement — deux actions indépendantes qui ne doivent jamais être couplées.
