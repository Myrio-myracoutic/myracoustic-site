...
export function isDateInBookingWindow(dateStr) {
  return dateStr >= todayInParis() && dateStr <= maxBookableDate();
}

function generateCandidateSlots(ranges) {
  const slots = [];
  for (const { start_time, end_time } of ranges) {
    let cur = start_time.slice(0, 5); // "09:00:00" -> "09:00"
    const end = end_time.slice(0, 5);
    while (cur < end) {
      const next = addMinutesToTime(cur, SLOT_MINUTES);
      if (next > end) break;
      slots.push(cur);
      cur = next;
    }
...
export async function getAvailableSlots(dateStr) {
  if (!isDateInBookingWindow(dateStr)) return [];
  ...
  let slots = generateCandidateSlots(ranges || []);
  ...
  try {
    ...
    if (busy.length) {
      slots = slots.filter(s => {
        ...
        const slotEnd = new Date(parisLocalToUtcISO(dateStr, addMinutesToTime(s, SLOT_MINUTES))).getTime();
        return !busy.some(b => {
